	...
	ID3D11Device* device)
{
	...
	for (auto& mtl : materials)
	{
		...
		if (mtl.map_Kd.size())
		{
			...
			hr = DirectX::CreateWICTextureFromFile(device, L"wood.jpg", &mtl.map_Kd_Tex, &mtl.map_Kd_TexSRV);
		...
		//	hr = DirectX::CreateWICTextureFromFile(device, L"metal.jpg", &mtl.map_bump_Tex, &mtl.map_bump_TexSRV);
