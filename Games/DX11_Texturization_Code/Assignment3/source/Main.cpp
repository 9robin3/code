...
void initObjects()
{
	...
	camera = new camera_t(fPI/4,				/*field-of-view*/
						...
						700.0f                /*z-far plane (everything further will be clipped/removed)*/
						);				
	camera->moveTo({ 0, 0, 7 });
	// create objects
...
void renderObjects()
{
	...
	//Obj
	obj->MapMatrixBuffers(g_DeviceContext, g_MatrixBuffer, Mcube * Mcube2 * quad_matrix, Mview, Mproj);
	obj->MapMaterialBuffers(g_DeviceContext, material_buffer, { 0.5, 0.5f, 0.5f, 0 }, { 0.5f, 0.5f, 0.5f, 0.2f }, { 0.5f, 0.5f, 0.5f, 0 });
